...
elseif (CMAKE_SYSTEM_PROCESSOR MATCHES "loongarch64")
    message(STATUS "LoongArch detected")
    # Auto-detect LASX/LSX from /proc/cpuinfo. Skip detection when
    # cross-compiling because /proc/cpuinfo describes the build host.
    if(CMAKE_CROSSCOMPILING)
        message(STATUS "Cross-compiling: skipping ISA auto-detection from /proc/cpuinfo")
        message(STATUS "compile loongarch with scalar")
        set(MARCH_FLAGS -mno-lsx)
    elseif(EXISTS /proc/cpuinfo)
        file(READ /proc/cpuinfo _cpuinfo)
        if(_cpuinfo MATCHES "lasx")
            message(STATUS "LoongArch LASX extension detected")
            set(MARCH_FLAGS -mlasx)
        elseif(_cpuinfo MATCHES "lsx")
            message(STATUS "LoongArch LSX extension detected")
            set(MARCH_FLAGS -mlsx)
        else()
            message(STATUS "compile loongarch with scalar")
            set(MARCH_FLAGS -mno-lsx)
        endif()
    ...
    message(FATAL_ERROR "vLLM CPU backend requires X86, Power9+ ISA, S390X ISA, ARMv8, RISC-V, or LoongArch support.")
endif()
